...
var keyCount=0
function preload(){
  bgImage = loadImage("keys/happy-fathers-day.jpg");
...
function setup(){
  ...
  greenkey.scale=0.1;
  bkey=createSprite(displayWidth/2,displayHeight/2,50,50);
  bkey.addImage(bimage);
  bkey.visible=false;
  bkey.scale=0.6;
  bukey=createSprite(displayWidth/2,displayHeight/2,50,50);
  bukey.addImage(bluimage);
  bukey.visible=false;
  bukey.scale=0.5;
  gkey=createSprite(displayWidth/2,displayHeight/2,50,50);
  gkey.addImage(gimage);
  gkey.visible=false;
  gkey.scale=0.5;
  rkey=createSprite(displayWidth/2,displayHeight/2,50,50);
  rkey.addImage(rimage);
  rkey.visible=false;
  rkey.scale=0.5;
  for(var i = 0; i<wallGroup.length;i++){
    wallGroup.get(i).shapeColor = "black";
  }
}
